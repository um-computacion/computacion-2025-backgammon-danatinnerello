    ...
    def manejar_eventos(self, dt):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                ficha_en_barra = self.juego.mostrar_tablero().mostrar_barra()[color_jugador]
                todas_en_casa = self.juego.mostrar_tablero().todas_en_ultimo_cuadrante(color_jugador)
                ...
                if ficha_en_barra:
                    if punto_click is not None:
                        ...
                        self.punto_origen = None
                        self._actualizar_mensaje("Ficha en barra. Debe reingresar. Click en el punto destino.", 1500)
                ...
                # PRIORIDAD 2: SACAR FICHA (SI TODAS ESTÁN EN CASA)
                    ...
                    if self.punto_origen is not None:
                         ...
                         self._actualizar_mensaje("Selecciona una ficha para sacarla o un movimiento normal.", 1500)
        ...
        if self.juego.__dados__.obtener_tiradas_restantes():
            if not self.juego.hay_movimientos_posibles(jugador):
                ...
                if ganador:
                    ...
                    self._actualizar_mensaje(f"¡Ganó {ganador.obtener_nombre()}!", self.duracion_mensaje_final)
                    self.tiempo_fin_juego = pygame.time.get_ticks() 
                else:
                    self._actualizar_mensaje("No hay movimientos posibles. Se pasa el turno.", 1800)
            else:
                self._actualizar_mensaje("Aún tienes movimientos posibles.", 1500)
        else:
            self._actualizar_mensaje("Ya no quedan tiradas disponibles. El turno terminó.", 1500)
    ...
    def _manejar_sacar_ficha(self, punto_origen, jugador):
        try:
            ...
            if ganador:
                self.ganador = ganador
                self._actualizar_mensaje(f"¡Ganó {ganador.obtener_nombre()}!", self.duracion_mensaje_final)
                self.tiempo_fin_juego = pygame.time.get_ticks() # Guardar tiempo para el cierre
    ...
    def _manejar_reingreso(self, punto_destino, jugador, color):
        try:
            ...
            if not dados_restantes:
                raise MovimientoInvalidoError("No tienes dados disponibles para reingresar.")

            dado_elegido = None
            # Lógica para encontrar el dado correcto
            for dado in sorted(dados_restantes, reverse=True):
                destino_calc = 24 - dado if color == "Blanca" else dado - 1 # INDEX (0-23)
                if destino_calc == punto_destino - 1:
                    dado_elegido = dado
                    break 

            if dado_elegido is None:
                raise MovimientoInvalidoError(f"No hay dado que entre al punto {punto_destino}.")
            ...
            color_jugador = jugador.obtener_color()
            barra_tiene_fichas = self.juego.mostrar_tablero().mostrar_barra()[color_jugador]
            ...
            # Si el core cambió el turno (porque no quedan dados), actualizamos el mensaje.
                ...
                self._actualizar_mensaje(
                    f"Turno de {self.juego.mostrar_turno().obtener_nombre()}", 
                    1500
                )
            else:
                # Si aún quedan dados y no pasó el turno
                mensaje = f"Reingreso exitoso con dado {dado_elegido}"
                if captura: mensaje = f"¡Capturaste ficha al reingresar con dado {dado_elegido}!"
        ...
        try:
            if self.punto_origen is None:
                ...
                if not self.juego.__dados__.obtener_tiradas_restantes():
                    ...
                    if captura: mensaje = f"¡Capturaste una ficha enemiga! (Usaste: {dados_str})"
                    self._actualizar_mensaje(mensaje, 1500)
