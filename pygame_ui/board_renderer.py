...
class TableroGrafico:
    """Representación gráfica del tablero y utilidades para mapear clicks a puntos."""

    def __init__(self, pantalla, alto_tablero=None):
        """Inicializa dimensiones y tamaños relativos usados para dibujar."""
    ...
    def dibujar_tablero(self):
        """Dibuja la estructura estática del tablero: triángulos, barras y fondo."""
        ...
        pygame.draw.rect(
            ...
            color_barra_central,
            pygame.Rect(
                self.ancho - self.ancho_barra // 2, 0, self.ancho_barra // 2, self.alto
            ),
        )
        pygame.draw.rect(
            self.pantalla,
    ...
    def dibujar_fichas(self, estado: dict):
        """Dibuja las fichas según el estado proporcionado por estado_desde_board.

        Args:
            estado (dict): mapeo punto -> {'color': str, 'cantidad': int}
        """
        ...
        for punto, datos in estado.items():
            ...
            if punto <= 12:
                if punto <= 6:
                    x = (
                        (6 - punto) * self.ancho_triangulo
                        + self.ancho_triangulo // 2
                        + self.ancho_barra // 2
                        + (7 * self.ancho_triangulo)
                    )
                else:
                    x = (
                        (12 - punto) * self.ancho_triangulo
                        + self.ancho_triangulo // 2
                        + self.ancho_barra // 2
                    )
                ...
                if punto <= 18:
                    x = (
                        (punto - 13) * self.ancho_triangulo
                        + self.ancho_triangulo // 2
                        + self.ancho_barra // 2
                    )
                else:
                    x = (
                        (punto - 19) * self.ancho_triangulo
                        + self.ancho_triangulo // 2
                        + self.ancho_barra // 2
                        + (7 * self.ancho_triangulo)
                    )
            ...
            for i in range(visibles):
                ...
                pygame.draw.circle(
                    self.pantalla, (0, 0, 0), (x, y), self.radio_ficha, 2
                )
    ...
    def dibujar_barra(self, tablero):
        """Dibuja las fichas ubicadas en la barra central del tablero."""
        ...
        for color, fichas in barra.items():
            ...
            radio = max(
                self.radio_ficha * 0.6,
                min(self.radio_ficha, alto_zona / (cantidad * 2)),
            )
            step = radio * 2
            ...
            for i in range(cantidad):
                y = (
                    (self.alto // 2) - ((i + 1) * step)
                    if color == "Blanca"
                    else (self.alto // 2) + (i * step) + step
                )
                pygame.draw.circle(
                    self.pantalla, color_rgb, (x_centro, int(y)), int(radio)
                )
                pygame.draw.circle(
                    self.pantalla, (0, 0, 0), (x_centro, int(y)), int(radio), 2
                )

            # Muestra número si hay más de 6
            if cantidad > 6:
                texto = pygame.font.Font(None, 28).render(
                    str(cantidad), True, (255, 0, 0)
                )
                self.pantalla.blit(
                    texto,
                    (
                        x_centro - 8,
                        (self.alto // 2) + (40 if color == "Negra" else -60),
                    ),
                )

    def dibujar_barra_lateral(self, tablero):
        """Dibuja la representación lateral de fichas fuera (afuera) del tablero."""
        ...
        pygame.draw.rect(
            self.pantalla,
            (255, 255, 255),
            texto_blanca_rect.inflate(16, 10),
            border_radius=6,
        )
        ...
        pygame.draw.rect(
            self.pantalla, (0, 0, 0), texto_negra_rect.inflate(16, 10), border_radius=6
        )
        self.pantalla.blit(texto_negra, texto_negra_rect)

    def obtener_punto_desde_click_en_ficha(self, pos, estado: dict):
        """Devuelve el punto si el click cae sobre la ficha superior de ese punto."""
        x_click, y_click = pos
        max_visibles = 5
    ...
    def obtener_punto_desde_click(self, pos):
        """Mapea un click (x,y) a un número de punto (1-24) o None si fuera de la zona."""
        x, y = pos
    ...
    def obtener_barra_lateral_desde_click(self, pos):
        """Detecta clicks en las barras laterales (izquierda/derecha) y devuelve la zona."""
        x, y = pos
...
def estado_desde_board(board):
    """Convierte la estructura interna del tablero a un dict sencillo para el renderer.

    Formato esperado:
      - board.__contenedor__ debe ser una lista de 24 "pilas" (listas) donde cada
        elemento puede contener objetos de ficha o strings representando color.
      - Cada pila representa las fichas en ese punto (índices 0..23).

    Retorna:
      dict donde cada clave es el número de punto (1-24) y el valor es otro dict:
        { punto: {"color": "Blanca"|"Negra", "cantidad": int} }

    Nota de ayuda:
      - El renderer solo necesita el color y la cantidad por punto. Si las fichas
        son objetos, se intenta llamar a obtener_color(); si son strings se usa su
        representación capitalizada.
      - Puntos vacíos se omiten en el dict resultante.
    """
    estado = {}
    ...
    for i, pila in enumerate(puntos):
        ...
        ficha = pila[0]
        color = (
            ficha.obtener_color().capitalize()
            if hasattr(ficha, "obtener_color")
            else str(ficha).capitalize()
        )
